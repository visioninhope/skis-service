      ...
      <div v-if="!sendComplete" id="contactOwnersMsg" class="row pt-2 pb-2 pl-4 pr-4" data-cy="contactOwnersMsg">
        <b-form-textarea type="text" id="approvalRequiredMsg"
                      v-model="contactOwnersMsg"
                       ...
                       class="form-control"/>
        <div :class="{ 'float-right':true, 'text-small': true, 'text-danger': charactersRemaining < 0 }" data-cy="charactersRemaining">{{ $filters.formatNum(charactersRemaining) }} characters remaining <i v-if="charactersRemaining < 0" class="fas fa-exclamation-circle"/></div>
